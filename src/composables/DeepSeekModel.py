...
Date: 2026-09-18 16:43:06
LastEditors: lujinwei lujinwei@hikvision.com.cn
LastEditTime: 2026-09-18 16:43:17
Description: 
'''
"""
DeepSeekModel.py — DeepSeek 大模型调用（Python 方式）
============================================================
安装依赖：
...
运行方式：
    # 交互模式（默认问题）
    python src/composables/DeepSeekModel.py

    # 命令行传入问题
    python src/composables/DeepSeekModel.py "你好，请介绍一下你自己"

    # 通过后端 API 调用（JSON 模式）
    python src/composables/DeepSeekModel.py --json '{"message":"你好","temperature":0.7,"model":"deepseek-chat"}'
============================================================
"""
import os
import sys
...
if not API_KEY:
    print(json.dumps({"error": "未找到 DEEPSEEK_API_KEY，请检查 .env 文件"}))
    sys.exit(1)
...
def chat(message, temperature=0.7, model="deepseek-chat"):
    """调用 DeepSeek 大模型并返回回复"""
    llm = ChatOpenAI(
        model=model,
        api_key=API_KEY,
        base_url=BASE_URL,
